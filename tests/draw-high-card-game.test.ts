...
describe('draw high card game with chance tests', function() {
  it('plays a game', function() {
    const game: DrawHighCard = new DrawHighCard(4, 0.1);
        ...
        for (let i = 0; i < plays.length; i++) {
          const play = plays[i], done = isOver[i];
          expect(game.state.possibleSimultaneousActions.size, 'correct simultaneous actions')
            .to.equal((minHandLength - i) * (maxHandLength - i));
            
          expect(game.state.awaitsChance(), 'no chance before plays are chosen').equal(false);
          
          game.simpleChoose(play[0],play[1]);          
          expect(game.state.awaitsChance(), 'chance event will happen').equal(true);
